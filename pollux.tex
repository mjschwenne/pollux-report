...
This is the \LaTeX{} report accompanying my Rocq / Lean development for Pollux,
a verification project focused on the compatibility of data
descriptors. Unfortunately, my current advisor is leaving academia for an
industry position, so I'm unsure what the fate of Pollux is. I've given the
report a once over and I believe that everything is in pretty good shape.

If you're looking to resume working on Pollux, I'd recommend starting with
Section~\ref{sec:inter-parse}, which generally matches the current state of the
formalized development. It uses a format simpler than Protobuf but still
capturing many of the interesting features in Protobuf. One of the key
advantages here is that the relations presented are simpler, while mirroring the
structures described in Section~\ref{sec:comp-rel}.
