...
\begin{mathpar}
   \infer*[Right=Refl]{ }{v \sim{} v}

   \infer*[Right=Trans]{v_1 \sim v_2 \\ v_2 \sim v_3}{v_1 \sim v_3}
\end{mathpar}
...
\begin{mathpar}
    \infer*[Right=Add-Opt]{ }{(id, \tau, \mathtt{IMPLICIT}) \sim (id, \tau,
      \mathtt{OPTIONAL})}

    {\color{red}
    \infer*[Right=Rm-Opt]{ }{(id, \tau, \mathtt{OPTIONAL}) \sim (id, \tau,
      \mathtt{IMPLICIT})}}

    \infer*[Right=Add-Rep]{ }{(id, \tau, \mathtt{IMPLICIT}) \sim (id, \tau,
      \mathtt{REPEATED})} 
...
We can freely convert been the \texttt{string} and \texttt{bytes} types.

\begin{mathpar}
  \infer*[Right=Str-Byte]{ }{(id, \mathtt{string}, m) \sim (id, \mathtt{bytes}, m)} 

  \infer*[Right=Byte-Str]{ }{(id, \mathtt{bytes}, m) \sim (id, \mathtt{string}, m)} 
\end{mathpar}

\subsection{Integer Rules}

\begin{mathpar}
  \infer*[Right=Width-Promote-I]{ }{(id, \mathtt{int32}, m) \sim (id,
    \mathtt{int64}, m)}

  \infer*[Right=Width-Promote-U]{ }{(id, \mathtt{uint32}, m) \sim (id,
    \mathtt{uint64}, m)}

  \infer*[Right=Width-Promote-S]{ }{(id, \mathtt{sint32}, m) \sim (id,
    \mathtt{sint64}, m)}

  \\
  \infer*[Right=Bool-Promote]{t \in \{\texttt{int32}, \texttt{int64},
    \texttt{uint32}, \texttt{uint64}, \texttt{sint32}, \texttt{sint64}\}}{(id,
    \texttt{bool}, m) \sim (id, t, m)}

  {\color{red} \infer*[Right=Enum-Promote]{t \in \{\texttt{int32}, \texttt{int64},
      \texttt{uint32}, \texttt{uint64}, \texttt{sint32}, \texttt{sint64}\}}{(id,
      \mathtt{enum}, m) \sim (id, t, m)} }
\end{mathpar}

\subsection{Message Rules}

Messages become an interesting one since we depart from the tuple notation for a
bit.

\begin{mathpar}
  \infer*[Right=Msg-Depth]{v_1 \sim v_1' \\ v_2 \sim v_2' \\ \cdots \\ v_n \sim v_n'}{\{v_1,
    \dots, v_n\} \sim \{v_1', \dots, v_n'\}}
  \\
  \infer*[Right=Msg-Width]{n \le m}{\{v_1, \dots, v_n\} \sim \{v_1, \dots, v_n,
    \dots, v_m\}}

  \infer*[Right=Msg-Field]{\mathtt{msg}_1 \sim \mathtt{msg}_2}{(id, \mathtt{msg}_1,
    m) \sim (id, \mathtt{msg}_2, m)} 
\end{mathpar}
