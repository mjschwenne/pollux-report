...
\begin{align*}
  ...
  s &: \text{I don't actually know...} & d &: \text{decorator}
\end{align*}
...
allow any integer to be converted into any other integer type without needing 30
different rules, we've split the rules into two groups. The first group changes
the width of the integer, i.e. allowing for \texttt{int32} to become an
\texttt{int64} or vice-versa. The second group converts between unsigned, signed
and zig-zag integers at the same bit width. Any integer conversion can happen in
at most two applications of integer rules.
...
Generally speaking, we can introduce, eliminate {\color{red} (It's clear what
  ...
  writer wins. What happens when we eliminate a packed field?)} or change the
