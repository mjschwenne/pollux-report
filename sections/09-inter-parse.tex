...
Below is the Rocq definitions for a descriptor and message. Both use a
\texttt{gmap} from the \texttt{stdpp} library to view a descriptor or message
respectively as a thinly wrapped map. Notice that these are mutually inductive
types, with a \texttt{Field} able to hold a \texttt{Desc} for a nested message.
...
\begin{minted}{coq}
    ...
    Inductive Message : Set := MSG (vs : gmap Z Value)
    with
      Value :=
    | M_MSG (m : Message)
    | M_BOOL (b : bool)
...
need a modified version of Definition~\ref{def:simp-pc}. % FIXME: why is this number wrong?
...
\end{definition}\label{def:simp-pc}
...
\begin{mathpar}
  ...
  \infer[Insert]{d' = d[k \mapsto \mathtt{F\_INT}] \\ \langle m' : d \preceq m : d \rangle}{\langle m'[k \mapsto z]
    : d' \preceq m[k \mapsto z] : d' \rangle}

  \infer[Drop]{k \not \in \dom{d} \\ \langle m' : d \preceq m : d \rangle}{\langle m'[k \mapsto z] : d \preceq m : d \rangle}

  \infer[Add]{ k \not \in \dom{m} \\ d' = d[k \mapsto \mathtt{M\_INT}] \\ \langle m' : d \preceq m :
...
\begin{mathpar}
  ...
  \infer[M-Add]{m_1 : d_1 \preceq m_2 : d_2 \\ v : f \\\\ k \not \in \dom{m_1} \\ k \not \in
    \dom{d_1}}{ m_1 : d_1 \preceq m_2[k \mapsto v] : d[k \mapsto f]}

  \infer[M-Update]{m_1 : d_1 \preceq m_2 : d_2 \\\\ m[k] : d_1[k] \prec v : f \\ d_1[k] \propto f}{ m : d_1 \preceq m[k
