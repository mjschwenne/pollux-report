...
\begin{definition}[Intermediate Descriptor Well-Formedness]\label{def:inter-dwf}
  A descriptor $d$ is considered well-formed, denoted $d \checkmark$, if
  \begin{itemize}
...
\begin{definition}[Intermediate Value Well-Formedness]\label{def:inter-vwf}
  A message $m$ is well-formed, denoted $m \checkmark$, if
  \begin{itemize}
...
\begin{definition}[Schema Correct]
  A value $v$ is \emph{schema correct} with respect to a descriptor $d$ if
  \begin{itemize}
  \item Every value in $v$ is declared in $d$.
  \item Every field in $d$ id present in $v$.
  \item Every value in $v$ matches the type required by the corresponding field
    in $d$.
...
\begin{lemma}[Intermediate Schema Correct Equal]\label{lem:inter-sc-eq}
  \[
    \forall\ m_1, m_2, d_1, d_2.\ \langle m_1 : d_1 \preceq m_2 : d_2 \rangle
    \rightarrow d_1 = d_2 \rightarrow v_1 = v_2
  \] 
\end{lemma}
...
\begin{theorem}[Intermediate Schema Correct Round-trip]\label{thm:inter-schema-correct}
  \[
    \langle m_1 : d \rangle \rightarrow m_1 \checkmark \rightarrow \forall\ bs, \mathrm{ser}_d(m_1) =
    bs \rightarrow \exists\ m_2, \mathrm{par}_d(bs) = m_2 \wedge \langle m_1
    : d \preceq m_2 : d \rangle
  \]
\end{theorem}
...
\begin{theorem}[Intermediate Identity Compatibility]\label{thm:inter-id-compat}
  \[
    d \checkmark \rightarrow m_1 \checkmark \rightarrow \forall\ bs, \mathrm{ser}_d(m_1) =
    bs \rightarrow \exists\ m_2, \mathrm{par}_d(bs) = m_2 \wedge \langle m_1
    : d \preceq m_2 : d \rangle
  \] 
\end{theorem}
...
\begin{mathpar}
  {\color{red}
  \infer[D-Update]{ \dom{d_{mand}} \supseteq \dom{d_{mand}'} \\ \forall\ k \in
...
\begin{theorem}[Intermediate Compatibility]\label{thm:inter-compat}
  \[
    d_1 \checkmark \rightarrow m_1 \checkmark \rightarrow d_2 \checkmark
    \rightarrow d_1 \ll d_2 \rightarrow \forall\ bs, \mathrm{ser}_{d_1}(m_1) =
    bs \rightarrow \exists\ m_2, \mathrm{par}_{d_2}(bs) = m_2 \wedge \langle m_1
    : d_1 \preceq m_2 : d_2 \rangle
  \] 
\end{theorem}
